...
case "${1:-}" in
  start)
    ...
    if ! /sbin/iptables -nL ANANAS_OUT >/dev/null 2>&1; then
      ...
      /sbin/iptables -A ANANAS_OUT -d 10.23.42.17 -o eth0 -j ACCEPT
    ...
    if ! /sbin/iptables -nL ANANAS_IN >/dev/null 2>&1; then
      ...
      /sbin/iptables -A ANANAS_IN -s 10.23.42.17 -i eth0 -j ACCEPT
    ...
    /bin/busybox start-stop-daemon -S -b -m -p "$ANANAS_PID" -x "$ANANAS_BASE/launcher" -- \
      ...
      -listen 10.23.42.30:8742 -interface eth0 -prefix 10.23.42.0/24 \
      -peer 10.23.42.17 -uid 1000 -gid 100
    ;;
  stop)
